...
%   Detailed explanation goes here
    ...
    s = struct('theta_op_ranges', {}, 'r_op_ranges', {}, 'theta_op_points', {}, 'r_op_points', {}, 'f_op_points', {}, 'gamma_op_points', {}, 'matrices_A', {}, 'matrices_B', {});
    ...
    for i = 1:n_rs
        for j = 1:n_thetas
            ...
            op_point_f0_gamma0 = subs(model_equations.equlibrium_equations, [m g x1(t) x3(t)], [model_parameters.m model_parameters.g s(k).theta_op_points s(k).r_op_points]);
            s(k).f_op_points = double(rhs(vpa([0 1]*op_point_f0_gamma0)));
            s(k).gamma_op_points = double(rhs(vpa([1 0]*op_point_f0_gamma0)));
        
            % Calculate linearized matrices and save them
            s(k).matrices_A = double(vpa(subs(Jacobian_A, [m g b1 b2 x1(t) x2(t) x3(t) x4(t) tau(t)], [model_parameters.m model_parameters.g model_parameters.b1 model_parameters.b2 s(k).theta_op_points 0 s(k).r_op_points 0 s(k).gamma_op_points])));
